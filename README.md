...
插件在后台静默完成用量统计与模型切换，不发送提醒。v1.3.3 起新增 `/路由` 命令，可按窗口(UMO+人格)临时开关插件介入。
...
v1.3.3 新增。在已注册人格的窗口中发送 `/路由` 即可切换该窗口(UMO+人格)的路由启用状态：

| 状态 | 行为 |
|------|------|
| 开启 → 关闭 | 插件暂停介入此窗口，不再设置 `selected_provider`，回退到 astrbot 默认链路 |
| 关闭 → 开启 | 插件恢复介入，按配置的路由链切换模型 |

**使用限制**：
- 仅对已在插件中**显式注册了当前人格**（配置了对应 `persona_id` 的窗口）的窗口生效。
- 通用窗口（`persona_id` 留空）不视为"已注册人格"，无法使用本命令。
- 未解析到人格时无法使用。

**持久化**：禁用状态按 (UMO, 人格) 维度独立保存到 `usage_data.json`，重启不丢失，不随每日 0 点用量重置而改变。再次发送 `/路由` 可恢复。
...
10. v1.3.3 起，`/路由` 命令的禁用状态同样持久化在 `usage_data.json` 中，不随每日 0 点用量重置而改变，需再次发送 `/路由` 手动恢复。
