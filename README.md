...
**关于工具生成的图片**：上面两种模式针对的是**用户发来的图片**。如果图片是会话中途由生图插件产生的（例如 aiimg 画完图后让 bot 看图说话），这类请求由插件调用 `ensure_vision_provider` 接口处理——插件会把当前会话换成能看图的模型，无需额外配置。详见下方「🔌 跨插件协作」。
